...
-- italian as % of total for each neighborhood
...
SELECT borough,
        neighborhood,
        COUNT(id) as total,
        COUNT(CASE WHEN cuisine = 'Italian' THEN id END) as italian,
        100*COUNT(CASE WHEN cuisine = 'Italian' THEN id END)/COUNT(id)::float as pct_italian
FROM cuisines
GROUP BY 1,2
ORDER BY 5 DESC
